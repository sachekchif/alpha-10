...
import { ShieldAlert, Search, Filter, Eye, CheckCircle, XCircle, FileQuestion, AlertTriangle, X, FileText, UserCheck } from 'lucide-react';

const kycEntries = [
  { name: 'TechCorp Innovations Ltd', level: 'Tier 3', date: '2 Hrs Ago', status: 'Pending' },
  { name: 'Globex Manufacturing', level: 'Tier 3', date: '5 Hrs Ago', status: 'Pending' },
  { name: 'Alpha Retail Group', level: 'Tier 2', date: 'Yesterday', status: 'Pending' },
  { name: 'Nexus Capital Partners', level: 'Tier 3', date: '22 Apr 2025', status: 'Approved' },
  { name: 'XYZ Imports', level: 'Tier 1', date: '28 Apr 2025', status: 'Rejected' },
];
...
export default function CorporateCompliancePage() {
  ...
  const filtered = kycEntries.filter(e =>
    activeTab === 'aml' ? true : e.status.toLowerCase() === activeTab
  );
  ...
  return (
    <div className="flex flex-col gap-8 pb-12 w-full animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Corporate Compliance</h1>
          <p className="text-gray-500 text-sm mt-1">KYC verification and AML monitoring for corporate accounts</p>
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
        <div className="flex overflow-x-auto border-b border-gray-100 dark:border-gray-700 px-6 pt-4 bg-gray-50/50 dark:bg-gray-900/20">
          {(['pending', 'approved', 'rejected', 'aml'] as const).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-3 text-sm font-bold border-b-2 whitespace-nowrap transition-colors capitalize flex items-center gap-2 ${activeTab === tab ? (tab === 'aml' ? 'border-red-500 text-red-600 dark:text-red-400' : 'border-[#961A1C] text-[#961A1C]') : 'border-transparent text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'}`}>
              {tab === 'aml' && <AlertTriangle size={14} className={activeTab === 'aml' ? 'text-red-500' : 'text-gray-400'} />}
              {tab === 'pending' ? 'Pending KYC (3)' : tab === 'aml' ? 'AML Alerts' : tab.charAt(0).toUpperCase() + tab.slice(1)}
            </button>
          ))}
        </div>
        <div className="p-4 border-b border-gray-100 dark:border-gray-700 flex gap-4">
          <div className="relative w-full md:w-80">
            ...
            <input type="text" placeholder="Search by company name..." className="w-full bg-gray-50 dark:bg-gray-900/50 text-gray-900 dark:text-white border border-gray-200 dark:border-gray-700 py-2 pl-9 pr-4 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-[#961A1C]" />
          </div>
          <button className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition">
          ...
          {activeTab !== 'aml' ? (
            ...
            <div className="p-6">
              <div className="bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-900/30 rounded-xl p-6 mb-4">
                ...
                <p className="text-red-700 dark:text-red-300 text-sm">These corporate entities have triggered automated AML thresholds requiring immediate review.</p>
              </div>
              <div className="p-4 border border-red-200 dark:border-red-900/30 rounded-xl bg-red-50/50 dark:bg-red-900/5 flex justify-between items-center">
                <div>
                  <span className="text-xs font-bold bg-red-100 text-red-700 px-2 py-0.5 rounded mr-2">Velocity Violation</span>
                  <span className="font-semibold text-gray-900 dark:text-white text-sm">Nexus Capital Partners</span>
                  <p className="text-xs text-gray-500 mt-1">3 transfers exceeding ₦50M within 24 hours.</p>
                </div>
          ...
          <div className="w-full max-w-md bg-white dark:bg-gray-900 h-full shadow-2xl flex flex-col animate-in slide-in-from-right">
            <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center bg-gray-50/50 dark:bg-gray-800/50">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">Corporate • {selectedUser.level}</p>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
              <div>
                ...
                <div className="flex flex-col gap-3">
                  {[{ title: 'Certificate of Incorporation', icon: <FileText className="text-blue-500" /> }, { title: 'Directors ID Verification', icon: <UserCheck className="text-green-500" /> }, { title: 'Memorandum & Articles', icon: <FileText className="text-purple-500" /> }].map((doc, i) => (
