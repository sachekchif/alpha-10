...
const transactions = [
  { id: 'TXN-R01', user: 'David Lee', type: 'Transfer', amount: '₦ 150,000', direction: 'outflow', status: 'Success', date: 'Today, 10:45 AM' },
  { id: 'TXN-R02', user: 'Maria Garcia', type: 'Funding', amount: '₦ 50,000', direction: 'inflow', status: 'Pending', date: 'Today, 09:12 AM' },
  { id: 'TXN-R03', user: 'James Smith', type: 'Investment', amount: '₦ 500,000', direction: 'outflow', status: 'Failed', date: 'Yesterday, 11:20 AM' },
  { id: 'TXN-R04', user: 'Sarah Connor', type: 'Withdrawal', amount: '₦ 80,000', direction: 'outflow', status: 'Success', date: '01 May 2025' },
  { id: 'TXN-R05', user: 'Michael Chen', type: 'Funding', amount: '₦ 200,000', direction: 'inflow', status: 'Success', date: '30 Apr 2025' },
];

const statusStyles: Record<string, string> = { Success: 'text-green-600 bg-green-50 dark:bg-green-900/20 dark:text-green-400', Pending: 'text-yellow-600 bg-yellow-50 dark:bg-yellow-900/20 dark:text-yellow-400', Failed: 'text-red-600 bg-red-50 dark:bg-red-900/20 dark:text-red-400' };
  ...
  return (
    <div className="flex flex-col gap-8 pb-12 w-full animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Retail Transactions</h1>
          <p className="text-gray-500 text-sm mt-1">All transactions across retail user accounts</p>
        </div>
        <button className="flex items-center gap-2 px-4 py-2.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 font-semibold rounded-lg text-sm transition hover:bg-gray-50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm p-5">
          ...
          <p className="text-2xl font-bold text-red-600">₦ 730,000</p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-gray-100 dark:border-gray-700 flex flex-col md:flex-row gap-4 justify-between">
          <div className="relative w-full md:w-80">
            ...
            <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search user or transaction ID..." className="w-full bg-gray-50 dark:bg-gray-900/50 text-gray-900 dark:text-white border border-gray-200 dark:border-gray-700 py-2 pl-9 pr-4 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-[#961A1C]" />
          </div>
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700/50">
              {filtered.map(t => (
                <tr key={t.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition cursor-pointer">
                  ...
                  <td className="px-6 py-4 font-semibold text-gray-900 dark:text-white">{t.user}</td>
                  ...
                  <td className="px-6 py-4">
                    <span className={`flex items-center gap-1 text-xs font-bold w-fit px-2.5 py-1 rounded-md ${t.direction === 'inflow' ? 'text-green-600 bg-green-50 dark:bg-green-900/20 dark:text-green-400' : 'text-red-600 bg-red-50 dark:bg-red-900/20 dark:text-red-400'}`}>
                      {t.direction === 'inflow' ? <ArrowUpRight size={12} /> : <ArrowDownRight size={12} />} {t.direction}
                    </span>
                  </td>
